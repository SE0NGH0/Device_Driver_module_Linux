...
int play_rps_game(client_info_t *c0, client_info_t *c1) {
    const char *prompt = "RPS 가위바위보! rock/paper/scissors 중 하나를 입력하세요:\n";
    ...
    while(1) {
        ...
        strncpy(buf0, r0.buf, BUF_SIZE); strncpy(buf1, r1.buf, BUF_SIZE);
        buf0[strcspn(buf0,"\r\n")] = '\0';
        buf1[strcspn(buf1,"\r\n")] = '\0';
        ...
        // (m0 - m1 + 3) % 3 == 1 -> c0 wins
        if((m0 - m1 + 3) % 3 == 1) return c0->player_id;
        else return c1->player_id;
    }
}
...
int main(){
    ...
    if(pf){int old; fscanf(pf,"%d",&old);
        if(kill(old,0)==0){kill(old,SIGTERM);sleep(1);} fclose(pf);remove(PID_FILE);}    
    pf=fopen(PID_FILE,"w");
